...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
 ...
 if (ParticleName == "opticalphoton" 
      && thePostPV->GetName() == "pmt_tube")//esto asumiendo que todos los fotones son cherenkov, pues en el detector construction se quitaron las propiedades de centelleo
  {
    alpmt++; //llegan al pmt
    phoOnPmt+=pmtSD->ProcessHits(step); //son detectados
    track->SetTrackStatus(fStopAndKill);
    //analysisManager->FillH1(1,alpmt);
